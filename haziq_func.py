...
def save_cookies(cookies):
    with open("/tmp/TK_cookies.json", "w") as file:
        json.dump(cookies, file, indent=4)


def check_expiry(accountname):
    with open(f"/data_midia/tk_haziq_cookies_{accountname}.json", "r") as file:
...
def _cookie_file(accountname):
    return f"/data_midia/tk_haziq_cookies_{accountname}.json"
...
def _load_or_create_cookies(accountname, proxy):
    ...
    if not cookie_read:
        ...
        os.rename("/tmp/TK_cookies.json", cookies_path)
